 ...
 *     test nothing. It also implements `remove()` with WordPress's
 *     (hook, callback, priority) matching, so `removeCoreCorsHeaders()`
 *     is exercised for real rather than asserted about.
 *
 *   - `WpCore` reproduces WP's `rest_send_cors_headers()`
...
namespace BCC\Trust\Core\Tests\Support {
    ...
    if (!class_exists(Hooks::class, false)) {
        ...
        final class Hooks
        {
            ...
            /**
             * WordPress's `remove_filter()`: matches on (hook, callback,
             * priority) and returns whether anything was actually removed.
             * Getting any of the three wrong is a silent no-op in production
             * too, which is exactly the failure mode worth reproducing.
             *
             * @param callable|string $cb
             */
            public static function remove(string $hook, $cb, int $priority): bool
            {
                $kept    = [];
                $removed = false;
                foreach (self::$hooks[$hook] ?? [] as $entry) {
                    if (!$removed && $entry['priority'] === $priority && $entry['cb'] === $cb) {
                        $removed = true;
                        continue;
                    }
                    $kept[] = $entry;
                }
                self::$hooks[$hook] = $kept;
            ...
            /** Fire an action: every callback, in WP order, no value threading. */
            public static function doAction(string $hook): void
            {
                $entries = self::$hooks[$hook] ?? [];
                usort(
                    $entries,
    ...
    if (!class_exists(WpCore::class, false)) {
        /**
         * WP core behaviour, reproduced.
         *
         * `sendCorsHeaders` is `rest_send_cors_headers()` verbatim
         * (wp-includes/rest-api.php) — unconditional reflection of any
        ...
        final class WpCore
        {
            ...
            /**
             * What `rest_api_default_filters()` does for CORS: register the
             * emitter on `rest_pre_serve_request` at priority 10. Hooked to
             * `rest_api_init` at 10, which is why the removal must run at 11.
             */
            public static function registerCorsFilter(): void
            {
                Hooks::add('rest_pre_serve_request', 'rest_send_cors_headers', 10, 1);
            }
        }
    }
...
namespace {
    ...
    if (!function_exists('rest_send_cors_headers')) {
        /**
         * Core's emitter, under its real global name.
         *
         * The name matters as much as the behaviour: `remove_filter()`
         * matches callbacks by identity, and core registers this one as the
         * plain string `'rest_send_cors_headers'`. Registering the stub
         * under any other identity would make the removal test pass against
         * code that removes nothing.
         *
         * @param  mixed $value
         * @return mixed
         */
        function rest_send_cors_headers($value = null)
        {
            return \BCC\Trust\Core\Tests\Support\WpCore::sendCorsHeaders($value);
        }
    }
}
...
namespace BCC\Trust\Core\Support {
    ...
    if (!function_exists('BCC\\Trust\\Core\\Support\\remove_filter')) {
        /** @param callable|string $cb */
        function remove_filter(string $hook, $cb, int $priority = 10): bool
        {
            return Hooks::remove($hook, $cb, $priority);
        }
    }
