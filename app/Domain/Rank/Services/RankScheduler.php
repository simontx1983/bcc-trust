...
final class RankScheduler
{
    ...
    public static function runPollCloseSweep(): void
    {
        update_option('bcc_rank_poll_close_sweep_last_run', time(), false);

        try {
            \BCC\Trust\Core\Plugin::instance()->pollService()->closeDuePolls();
            update_option('bcc_rank_poll_close_sweep_last_success', time(), false);
        } catch (\Throwable $e) {
