...
class ScoreRepository {
    ...
    /**
     * Get top scored pages with config-based ordering
     *
     * @return stdClass[]
     */
    public function getTopScored(int $limit = 10, string $orderBy = 'total_score'): array {
        ...
        $allowedOrders = ['total_score', 'confidence_score', 'vote_count', 'positive_score', 'endorsement_count'];
        ...
        $results = $wpdb->get_results(
            $wpdb->prepare(
                "SELECT s.page_id, s.category_id, s.page_owner_id, s.total_score, s.onchain_bonus, s.endorsement_bonus, s.positive_score, s.negative_score, s.vote_count, s.unique_voters, s.confidence_score, s.reputation_tier, s.endorsement_count, s.last_vote_at, s.last_calculated_at, s.fraud_metadata, s.recalculate_required, p.post_title, u.display_name as owner_name
                 FROM {$this->table} s
